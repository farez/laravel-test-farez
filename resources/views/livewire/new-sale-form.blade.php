    ...
    <form wire:submit.prevent="createSale">
        <div class="grid grid-cols-4 gap-4 align-baseline">
            ...
            <div>
                ...
                <div class="mt-1">
                    <div wire:loading wire:target="quantity, unitCost" class="inline-block align-middle mt-2">{{ __('Calculating...') }}</div>
                    <div wire:loading.remove wire:target="quantity, unitCost" id="sellingPrice" class="inline-block align-middle mt-2 w-full sm:text-sm border-0 disabled">{{ number_format($this->sellingPrice, 2) }}</div>
                </div>
            </div>
