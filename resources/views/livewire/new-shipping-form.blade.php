<div class="sm:w-3/4">
    <form wire:submit.prevent="createShippingCharge" class="flex flex-col sm:w-1/4">
        <div>
            <label for="charge" class="block text-sm font-medium text-gray-700"> {{ __('New cost of shipment') }} </label>
            <div class="mt-1">
                <input wire:model="charge" type="text" name="charge" id="charge" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
            </div>
            @error('charge') <span class="text-red-700">{{ $message }}</span> @enderror
        </div>
        <div class="mt-2">
